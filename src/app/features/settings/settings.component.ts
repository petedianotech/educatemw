  ...
  template: `
    <div class="min-h-screen bg-slate-50 pb-safe">
      ...
      <header class="px-4 py-3 flex items-center gap-3 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm sticky top-0 z-10 pt-safe">
        ...
        <h1 class="text-xl font-bold text-slate-900">App Settings</h1>
      </header>
      ...
      <div class="p-4 max-w-2xl mx-auto space-y-4 mt-2 overflow-y-auto h-[calc(100vh-80px)] custom-scrollbar">
        ...
        <section class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
          <div class="p-5 border-b border-slate-50 flex items-center justify-between">
            <h3 class="font-black text-slate-900 text-sm flex items-center gap-2">
              <mat-icon class="text-indigo-600 text-sm">palette</mat-icon>
              App Theme
            </h3>
          ...
          <div class="p-5">
            <div class="flex gap-3">
              <button class="flex-1 p-4 rounded-2xl border-2 border-indigo-600 bg-indigo-50 flex flex-col items-center gap-2 transition-all">
                <mat-icon class="text-indigo-600">light_mode</mat-icon>
                <span class="text-[10px] font-black uppercase tracking-widest text-indigo-900">Classic</span>
              </button>
              <button (click)="alertMock('Dark mode coming soon to Educate MW!')" class="flex-1 p-4 rounded-2xl border border-slate-200 bg-white flex flex-col items-center gap-2 hover:bg-slate-50 transition-all active:scale-95 group">
                <mat-icon class="text-slate-400 group-hover:text-indigo-600">dark_mode</mat-icon>
                <span class="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-indigo-900">Dark</span>
              </button>
            </div>
          </div>
        </section>
        ...
        <section class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
          ...
          <div class="p-5 space-y-4">
            ...
            <a href="https://wa.me/265987066051?text=Hello%20Peter,%20I%20am%20reporting%20an%20ad%20in%20Educate%20MW." 
               target="_blank"
               class="w-full py-4 bg-emerald-500 text-white rounded-xl font-black shadow-lg shadow-emerald-100 hover:bg-emerald-600 transition-all active:scale-[0.98] flex items-center justify-center gap-3 text-xs">
              Report Ads to Peter
            </a>
          </div>
        </section>
        ...
        <section class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
          ...
          <div class="p-5">
            @if (!isAdmin()) {
              <div class="space-y-4">
                <p class="text-[10px] text-slate-500 font-medium leading-tight px-1 uppercase tracking-widest text-center">Admin Team Only</p>
                <div class="flex flex-col gap-3">
                  <input type="password" [(ngModel)]="adminPassword" placeholder="Enter Team Password" 
                         class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-bold placeholder-slate-400 focus:outline-none focus:border-indigo-500 transition-all outline-none text-sm text-slate-700 shadow-sm">
              ...
              <div class="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex flex-col items-center text-center gap-3">
                <mat-icon class="text-indigo-600 !w-10 !h-10 !text-[40px]">verified_user</mat-icon>
                <div>
                  <p class="text-indigo-900 font-black text-sm">Admin Access Active</p>
                  <p class="text-indigo-600 text-[10px] font-bold mt-1">You can now access the Admin Dashboard.</p>
                </div>
