    ...
    <div class="h-full overflow-y-auto bg-slate-50 pb-24 relative">
      ...
      <div class="relative z-10 p-4 md:p-8">
        ...
        <div class="grid grid-cols-2 gap-4 mb-8 max-w-2xl mx-auto">
          ...
          <!-- Cleo AI -->
          <a routerLink="/chat" class="card-modern p-5 flex flex-col justify-between aspect-square relative overflow-hidden group">
            <div class="absolute -right-4 -top-4 w-24 h-24 bg-indigo-100 rounded-full blur-2xl opacity-60 group-hover:bg-indigo-200 transition-colors"></div>
            <div class="flex justify-between items-start relative z-10">
              <div class="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center border border-indigo-100 shadow-sm group-hover:scale-110 transition-transform duration-300">
              ...
              @if (!authService.currentUser()?.isPro && authService.currentUser()?.role !== 'admin' && (authService.currentUser()?.aiCredits ?? 5) <= 0) {
                <div class="bg-amber-50 text-amber-600 p-1.5 rounded-full border border-amber-200 shadow-sm">
                  <mat-icon class="!w-4 !h-4 !text-[16px]">lock</mat-icon>
                </div>
              }
            </div>
            <div class="relative z-10">
              <h3 class="font-bold text-xl mb-1 text-slate-800">Cleo AI</h3>
              <p class="text-slate-500 text-xs font-medium">Ask anything</p>
            </div>
          </a>
